import { Alchemy, Network, AssetTransfersParams, AssetTransfersResponse, AssetTransfersCategory, SortingOrder } from 'alchemy-sdk';
...
export const getTransactionHistory = async (address: string, pageKey?: string): Promise<{ transactions: Transaction[], pageKey: string | undefined }> => {
  try {
    ...
    const outgoingTransactions = outgoingResponse.transfers.map(tx => ({
      ...
      operationType: tx.to && tx.to.toLowerCase() === '0x1231deb6f5749ef6ce6943a275a1d3e7486f4eae' ? 'Swap' : 'Send',
    })) as Transaction[];
    ...
    const pageKeyCombined = outgoingResponse.pageKey || incomingResponse.pageKey;
