 ...
 */
export function getCallDataTransfer(
    recipientAddress: string,
    tokenAddress: string,
    amount: string | ethers.BigNumber,
): Exclude<UserOperationCallData, string> {
    // Ensure amount is treated as a BigNumber
    const amountBigNumber = ethers.BigNumber.isBigNumber(amount)
        ? amount
        : ethers.BigNumber.from(amount);

    const calldata = new ethers.utils.Interface(abiERC20).encodeFunctionData(
        "transfer",
        [recipientAddress, amountBigNumber],
    ) as `0x${string}`;
