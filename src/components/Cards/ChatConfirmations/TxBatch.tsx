  ...
  return (
    <div className="bg-white rounded-[20px] shadow-sm p-4 w-[454px]">
      <div className="p-[24px]">
        <h1 className="pb-[24px] text-center" style={{ fontSize: '20px', fontFamily: 'DM Sans' }}>
          Selected Tokens
        </h1>
        {tokens.map((token, index) => (
          <div key={index} className="flex justify-between items-center mb-2">
            <div className="flex items-center">
              <div className="mr-2">
                <Image src={token.logo} width={24} height={24} alt={token.name} />
              </div>
...
const TxBatch = ({ tokens, percentages, priceImpact, networkCost, maxSlippage }) => {
  return (
    <div>
      <TxBatchHeader tokens={tokens} percentages={percentages} />
      <TxSummaryDetails priceImpact={priceImpact} networkCost={networkCost} maxSlippage={maxSlippage} />
    </div>
  );
};
